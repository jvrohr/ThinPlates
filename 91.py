...
class plate:
    ...
    def analysis(self, p0, N, m_max_evaluation, n_max_evaluation,
                 ...
                 printProcess=False, folderName='fig'):
        ...
        if typeOfAnalysis == 'together':
            ...
            if converGraph:
                ...
                plt.plot(mn_values[0:k+1], wEval[0:k+1], linewidth=1.5)
                ...
                plt.grid(b=True, which='major', color='gray', linestyle='-',
                         linewidth=0.1)
                plt.minorticks_on()
                plt.savefig(folderName + '\\91_convergencia.png')
...
N = 500e3  # [N/m] Carga lateral distribuída sob a placa
...
X, Y = np.meshgrid(X, Y)
w, m_conv, n_conv = myPlate.analysis(p0, N, M_MAX, N_MAX, relError=5e-4,
...
for i in range(pointsN):
    w, m_conv, n_conv = myPlate.analysis(p0, N_new[i], M_MAX, N_MAX,
                                         converGraph=False, printProcess=False)
    W[i] = w(P_aval[0], P_aval[1])

# Gráfico
fig = plt.figure('VariacaoN')
plt.plot(N_new, 1e3*W)
plt.xlabel('Força distribuída N')
...
for i in range(pointsAB):
    ...
    w, m_conv, n_conv = myPlate.analysis(p0, N, M_MAX, N_MAX,
                                         converGraph=False, printProcess=False)
    W[i] = w(P_aval[0], P_aval[1])
