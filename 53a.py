        ...
        def w(x, y):
            ...
            for m in np.array(range(int((m_max+1)/2)))*2 + 1:
                for n in np.array(range(int((n_max+1)/2)))*2 + 1:
                    Qmn = 4*p*np.sin(m*np.pi*area[0]/self.a)*np.sin(n*np.pi*area[1]/self.b)*np.sin(m*np.pi*area[2]/self.a)*np.sin(n*np.pi*area[3]/self.b)/(np.pi**2*m*n*area[2]*area[3])
...
x1 = a/3 # [m] Posição em x do centro da área de aplicação da carga
y1 = b/3 # [m] Posição em y do centro da área de aplicação da carga
c = a/6  # [m] Largura (em x) da área de aplicação da carga
d = b/6  # [m] Comprimento (em y) da área de aplicação da carga
...
plt.savefig(figDirectory + '\\53a_convergencia.png')
...
plt.savefig(figDirectory + '\\53a_deflexao3D.png')
...
plt.savefig(figDirectory + '\\53a_Momentos.png')
...
plt.savefig(figDirectory + '\\53a_Cortante.png')
...
plt.savefig(figDirectory + '\\53a_forcas3D.png')
